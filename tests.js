...
for (const f of ["data.js", "game.js", "lang-pt.js", "lang-pt2.js", "lang-en.js", "lang-en2.js"]) vm.runInContext(fs.readFileSync("src/" + f, "utf8").replace(/^"use strict";/, "").replace(/^const /gm, "var "), ctx);
...
const simsSrc = fs.readFileSync("src/sims.js", "utf8") + fs.readFileSync("src/sims2.js", "utf8"); for (const s in ctx.TASKS) { if (!simsSrc.includes("SIMS." + s)) fail("simulador " + s + " não implementado"); for (const tk of ctx.TASKS[s]) if (!simsSrc.includes('"' + (tk.k || tk.id) + '"')) fail("check da tarefa " + tk.id + " ausente"); }
